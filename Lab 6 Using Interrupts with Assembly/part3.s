...
_start:                                         
      ...
      LDR SP, =0x2000               //set up SP
               
      //also enable interrup in SVC mode 
      MOV R0, #0b1010011
      MSR CPSR, R0                  //change to SVC mode
      LDR SP, =0x3FFFFFFC           //set up SP
...
KEY_1: 
      //speed should doubled, half the timer value 
      LDR     R0, =TIMER_BASE
...
KEY_2:
    //speed should slow down by half, double the timer value ->2FAF080
      LDR     R0, =TIMER_BASE
      LDR     R1, [R0, #0x8] //get the lower count
      LDR     R2, [R0,#0xC] //get the higher count
...
SERVICE_UND:        B   SERVICE_UND       //Software interrupts
SERVICE_SVC:        B   SERVICE_SVC       //Aborted data reads
SERVICE_ABT_DATA:   B   SERVICE_ABT_DATA    //Aborted instruction fetch
...
CONFIG_GIC:
      ...
      STR   R1, [R0, #0x04]
      /* Set the enable bit in the CPU Interface Control Register (ICCICR). This bit allows interrupts to be forwarded to the CPU(s) */
      MOV   R1, #1
      STR   R1, [R0]

      /* Set the enable bit in the Distributor Control Register (ICDDCR). This bit allows the distributor to forward interrupts to the CPU interface(s) */
      LDR   R0, =0xFFFED000
      STR   R1, [R0]    
...
CONFIG_INTERRUPT:
      ...
      /* Configure Interrupt Set-Enable Registers (ICDISERn).  reg_offset = (integer_div(N / 32) * 4 value = 1 << (N mod 32) */
      ...
      /* now that we have the register address (R4) and value (R2), we need to set the correct bit in the GIC register */
      LDR   R3, [R4]                // read current register value
      ORR   R3, R3, R2              // set the enable bit
      STR   R3, [R4]                // store the new register value

      /* Configure Interrupt Processor Targets Register (ICDIPTRn) reg_offset = integer_div(N / 4) * 4 index = N mod 4 */
